...
export function PPPBasketComparison({
  ...
  return (
    <figure role="img" aria-label={`Perbandingan daya beli antara ${fromCity} dan ${toCity}`}>
      ...
      <figcaption className="sr-only">
        Perbandingan daya beli: {fromCity} {fromSalaryFormatted} vs {toCity} (PPP) {toSalaryFormatted}.
        {isBetter ? `${toCity} memiliki ${ratioDisplay}% lebih banyak daya beli` : `${fromCity} memiliki ${Math.abs(Number(ratioDisplay))}% lebih banyak daya beli`}.
      </figcaption>
    </figure>
  )
}
