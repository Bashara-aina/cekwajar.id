...
export function COLComparisonChart({
  ...
  return (
    <figure role="img" aria-label={`Perbandingan biaya hidup antara ${fromCity} dan ${toCity}`}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 text-center text-xs border-t pt-3">
          <div className="bg-muted/50 rounded-lg p-3">
            <p className="text-muted-foreground">Total {fromCity}</p>
            <p className="font-bold text-sm mt-0.5">Rp {formatIDR(totalFrom)}</p>
          </div>
          <div className="bg-muted/50 rounded-lg p-3">
            <p className="text-muted-foreground">Total {toCity}</p>
            <p className="font-bold text-sm mt-0.5">Rp {formatIDR(totalTo)}</p>
          </div>
        </div>
      </div>
      <figcaption className="sr-only">
        Perbandingan biaya hidup bulanan: {fromCity} Rp {formatIDR(totalFrom)} vs {toCity} Rp {formatIDR(totalTo)}.
        {isMoreExpensive ? `${toCity} lebih mahal ${diffPct}%` : `${toCity} lebih murah ${Math.abs(Number(diffPct))}%`}.
        Selisih: Rp {formatIDR(Math.abs(diff))}/bulan.
      </figcaption>
    </figure>
  )
}
