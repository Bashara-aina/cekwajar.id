...
export function PropertyPriceBar({ p25, p50, p75, upperFence, userPrice, verdict, pricePerM2 }: PropertyPriceBarProps) {
  ...
  return (
    <figure role="img" aria-label={`Perbandingan harga properti - Harga Anda: ${formatIDR(userPrice)}`}>
      ...
      <figcaption className="sr-only">
        Harga properti Anda {formatIDR(userPrice)} berada di zona {config.label}.
        Bandingkan dengan P25: {formatIDR(p25)}, Median: {formatIDR(p50)}, P75: {formatIDR(p75)}.
      </figcaption>
    </figure>
  )
}
