    ...
    return (
      <>
        ...
        <button
          ...
          className="hidden md:flex items-center gap-2 px-3 py-1.5 text-sm text-slate-500 dark:text-slate-500 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          aria-label="Open command menu"
        >
          <Search className="h-4 w-4" />
          <span className="text-xs">Search...</span>
          <kbd className="pointer-events-none hidden h-5 select-none items-center gap-1 rounded border border-slate-200 bg-slate-100 px-1.5 font-mono text-xs font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
            ⌘K
          </kbd>
        </button>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center border-b border-slate-100 px-3 dark:border-slate-800">
                <Search className="mr-2 h-4 w-4 shrink-0 text-slate-500 dark:text-slate-500" />
                <Command.Input
                  ref={inputRef}
                  value={search}
                  onValueChange={setSearch}
                  placeholder="Ketik perintah atau navigasi..."
                  className="flex h-12 w-full rounded-md bg-transparent py-3 text-sm outline-none placeholder:text-slate-500 dark:placeholder:text-slate-500 disabled:cursor-not-allowed disabled:opacity-50"
                  autoFocus
                />
                <button
                  onClick={() => { setOpen(false); setSearch('') }}
              ...
              <Command.List className="max-h-[320px] overflow-y-auto overflow-x-hidden p-2 scrollbar-hide">
                <Command.Empty className="py-6 text-center text-sm text-slate-500 dark:text-slate-500">
                  Tidak ada hasil.
                </Command.Empty>
                ...
                <Command.Group
                  heading="Navigasi"
                  className="px-2 py-2 text-xs font-medium text-slate-500 dark:text-slate-500"
                >
                  {NAV_ITEMS.map((item) => {
