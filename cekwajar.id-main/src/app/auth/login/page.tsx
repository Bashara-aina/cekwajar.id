    ...
    <Card className="border-slate-200 shadow-sm">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl font-bold">
          {isSignupIntent ? 'Buat akun cekwajar.id' : 'Masuk ke cekwajar.id'}
        </CardTitle>
        <CardDescription>
          {isSignupIntent
            ? 'Gratis selamanya. Langsung mulai setelah login.'
            : 'Gratis selamanya untuk fitur dasar.'}
        </CardDescription>
      </CardHeader>

      <CardContent className="space-y-4">
        {/* Magic link form first */}
        {magicLinkSent ? (
          ...
          <form onSubmit={handleMagicLink} className="space-y-3">
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? 'Mengirim...' : isSignupIntent ? 'Kirim Link' : 'Kirim Link Masuk'}
            </Button>
          </form>
        )}
        ...
        {/* Footer — conversion saver: push indecisive visitors back to value */}
        <div className="mt-6 border-t border-slate-100 pt-4">
          <p className="text-center text-xs text-slate-500">
            Tidak punya akun? Tidak perlu daftar —{' '}
            <Link href="/wajar-slip" className="font-semibold text-emerald-700 hover:underline">
              audit slip dulu, gratis →
            </Link>
          </p>
        </div>
...
function SearchParamsReader({ children, isSignupIntent }: { children: React.ReactNode; isSignupIntent: boolean }) {
  return (
    <>
      {children}
      {!isSignupIntent && (
