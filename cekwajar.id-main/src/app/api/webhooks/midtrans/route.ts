...
export async function POST(request: Request) {
  ...
  if (isActivation) {
    ...
    // Insert into payments table for refund endpoint
    await serviceClient.from('payments').insert({
      user_id: tx.user_id,
      midtrans_order_id: body.order_id,
      amount_idr: parseInt(body.gross_amount, 10),
      status: 'paid',
      paid_at: new Date().toISOString(),
    }).then(({ error: insertError }) => {
      if (insertError) console.error('Failed to insert payments record:', insertError)
    })
  }
